...
```{r}
# read in dataset 
Ponds.rel <- as.data.frame(read.csv("/Users/ericanadolski/GitHub/team2/Ponds.rel.csv", row.names=1, header = T, sep = ",", dec = ".")) 
...
#### code to subset sequencing libraries by grep
```{r, echo=FALSE}
...
### Scatter plot of some environmental variables to look at variation
```{r}
...
ggplot()+ 
  ...
  geom_text_repel(data = species.scores.abundant, aes(x = NMDS1, y = NMDS2, label = species), size = 3)

# try to fix the overlaps in text library("ggrepel")   

# add abundant species scores
ggplot()+ 
  geom_hline(yintercept = 0, linetype = "dashed")+
  geom_vline(xintercept = 0, linetype = "dashed")+
  geom_point(data = data.scores, mapping = aes(x = NMDS1, y = NMDS2, colour = location, shape = source), size=3, alpha = .6) +
  ...
  geom_segment(data = arrows.p, aes(x = 0, y = 0, xend = NMDS1, yend = NMDS2), 
               arrow = arrow(length = unit(.2, "cm")*arrows.p$R),  color = "grey")+
  geom_text_repel(data = arrows.p, aes(x = NMDS1, y = NMDS2, label = env), size=5)+
  geom_text_repel(data = species.scores.abundant, aes(x = NMDS1, y = NMDS2, label = species), size = 3)
...
pond.dbrda <- dbrda(species_mat ~ ., data = as.data.frame(env_scaled), na.action=na.omit) #perform dbRDA

# first model only intercept
pond.dbrda.mod0 <- dbrda(species_mat ~ 1, data = as.data.frame(env_scaled), na.action=na.omit) 

# now model with all explanatory variables
pond.dbrda.mod1 <- dbrda(species_mat ~ ., data = as.data.frame(env_scaled), na.action=na.omit)

# now iterate through all combos of explanatory variables, return model with lowest AIC
pond.dbrda <- ordiR2step(pond.dbrda.mod0,pond.dbrda.mod1, permutations = how(nperm = 999))
...
## plot output is odd because pond.vectors are values < 1 so they look tiny in comparison to site dbRDA values which are ~100s
## try RDA or NMDS instead of dbRDA, investigate HNF sites as outliers
